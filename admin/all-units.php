...
<body class="menu-collapsed">
								...
								<div class="panel-body">
									<div class="basic-datatable-block table-responsive">
										<table id="basic_datatable" class="display table table-bordered basic-data-table">
											<thead>
												<tr>
													<th class="view-message">Unit ID</th>
													<th class="view-message">Unit Name</th>
											...
											<?php
												...
												while($row = mysqli_fetch_array($result)){
													?>
													<tr id="<?php $row["unit_id"] ?>">
													<td><?php echo $row["unit_id"] ?></td>
													<td><?php echo $row["unit_name"]?></td>
													<!-- <td><?php echo $row["status"]?></td> -->
													<input type="hidden" id="unit_id" value=<?php $row["unit_id"]?>></input>

													<?php 
													if($row["status"] == '1'){
													?>
													<td><button id=<?php  echo $row["unit_id"] ?> href="#" data-role="unit_off" data-is=<?php  echo $row["unit_id"] ?> >Active</button></td>
													</tr>

													<?php 
														}else{
															?>
															<td><button id=<?php  echo $row["unit_id"] ?> href="#" data-role="unit_on" data-is=<?php  echo $row["unit_id"] ?> >In-active</button></td>
															</tr>
													<?php
														}
													?>
	...
	<script>
		$(document).ready(function(){
			...
			$(document).on('click','button[data-role=unit_off]',function(){
				// 	displaycount();
				var unit_id = this.id;
				var code = 0;
				//console.log("Disable",unit_id);

				$.post('./php_form/unit_status.php',{"id": unit_id, "code": code});
				// location.reload();

			});

			$(document).on('click','button[data-role=unit_on]',function(){
				// 	displaycount();
				var unit_id = this.id;
				var code = 1;
		    	//console.log("Enable", unit_id);

				$.post('./php_form/unit_status.php',{"id": unit_id, "code": code});
				// location.reload();
			});

			$('button[data-role=unit_off]').css({"background-color" : "#0be881", "border" : "0", "color" : "#fff", "width" : "80px"});
			$('button[data-role=unit_on]').css({"background-color" : "#ff3f34", "border" : "0", "color" : "#fff", "width" : "80px"});
