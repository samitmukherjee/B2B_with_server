...
if(empty($_SESSION['user']) && $_SESSION['user_type'] != 'admin'){
	...
	$sql = "SELECT s1.subcategory_id, c1.category_name, s1.subcategory_name, s1.status FROM subcategory_table s1, category_table c1 where s1.category_id = c1.category_id";
...
<body class="menu-collapsed">
								...
								<div class="panel-body">
									<div class="basic-datatable-block table-responsive">
										<table id="basic_datatable" class="display table table-bordered basic-data-table">
											<thead>
												<tr>
													<th class="view-message">Sub-Category ID</th>
													<th class="view-message">Parent Category</th>
													<th class="view-message">Sub-Category Name</th>
											...
											<?php
												...
												while($row = mysqli_fetch_array($result)){
													?>
													<tr id="<?php $row["subcategory_id"] ?>">
													<td><?php echo $row["subcategory_id"] ?></td>
													<td><?php echo $row["category_name"]?></td>
													<td><?php echo $row["subcategory_name"]?></td>
													<!-- <td><?php echo $row["status"]?></td> -->
													<input type="hidden" id="subcategory_id" value=<?php $row["subcategory_id"]?>></input>

													<?php 
													if($row["status"] == '1'){
													?>
													<td><button id=<?php  echo $row["subcategory_id"] ?> href="#" data-role="subcategories-on" data-is=<?php  echo $row["subcategory_id"] ?> >Active</button></td>
													</tr>

													<?php 
														}else{
															?>
															<td><button id=<?php  echo $row["subcategory_id"] ?> href="#" data-role="subcategories-off" data-is=<?php  echo $row["subcategory_id"] ?> >In-active</button></td>
															</tr>
													<?php
														}
													?>
	...
	<script>
		$(document).ready(function(){
			...
			$(document).on('click','button[data-role=subcategories-off]',function(){
				// 	displaycount();
				var subcategory_id = this.id;
				var code = 1;
				//console.log("Disable",category_id);

				$.post('./php_form/subcategory_status.php',{"subcategory_id": subcategory_id, "code": code},function(){
					location.reload();
				});
			...
			$(document).on('click','button[data-role=subcategories-on]',function(){
				// 	displaycount();
				var subcategory_id = this.id;
				var code = 0;
			...
			$('button[data-role=subcategories-off]').css({"background-color" : "#ff3f34", "border" : "0", "color" : "#fff", "width" : "80px"});
			$('button[data-role=subcategories-on]').css({"background-color" : "#0be881", "border" : "0", "color" : "#fff", "width" : "80px"});
