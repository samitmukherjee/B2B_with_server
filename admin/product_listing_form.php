<?php
    ...
    if(isset($_POST['submit_button']) && isset($_POST['business'])) {
        // echo $_POST['seller'];
        $seller = $_POST['business'];

        $sql = "SELECT business_id FROM business_details WHERE business_email_id = '$seller' ";
        //echo $sql;
        ...
        if(mysqli_num_rows($res)>0){
            ...
            $business_id =  $bid['business_id'];
                ...
                if(isset($_POST['product_name']) && isset($_POST['brand_name']) && isset($_POST['product_category']) && isset($_POST['price']) && isset($_POST['unit'])
                && isset($_POST['product_description']) && isset($_POST['short_description']) && isset($_FILES['brand_logo']) && isset($_POST['color'])
                && isset($_POST['discount']) && isset($_POST['quantity']) && isset($_POST['weight']) & isset($_POST['length']) && isset($_POST['width'])
                && isset($_POST['height']) && isset($_FILES['product_image_1'])  && isset($_FILES['product_image_2'])  && isset($_FILES['product_image_3'])){
                ...
                if(!$conn){
                    ...
                    $query_get_unit_id = "SELECT unit_id FROM unit_table WHERE unit_name = '$product_unit'";
                    $result_get_unit_id = mysqli_query($conn,$query_get_unit_id);
                    $row1 = mysqli_fetch_assoc($result_get_unit_id);
                    $product_unit = $row1['unit_id'];
                ...
                //header("product-listing.php");
                // echo '<meta http-equiv="refresh" content="1; URL=../product-listing.php" />';
            }
        } else{
            // header("location: ../dynamic-dashboard.php?*Couldnt add the product");
        }
    } else{
        // header("location: ../dynamic-dashboard.php");
    }
