...
<body class="menu-collapsed">
								...
								<div class="panel-body">
									<div class="basic-datatable-block table-responsive">
										<table id="basic_datatable" class="display table table-bordered basic-data-table">
											<thead>
												<tr>
													<th class="view-message">Category ID</th>
													<th class="view-message">Category Name</th>
											...
											<?php
												...
												while($row = mysqli_fetch_array($result)){
													?>
													<tr id="<?php $row["category_id"] ?>">
													<td><?php echo $row["category_id"] ?></td>
													<td><?php echo $row["category_name"]?></td>
													<!-- <td><?php echo $row["status"]?></td> -->
													<input type="hidden" id="category_id" value=<?php $row["category_id"]?>></input>

													<?php 
													if($row["status"] == '1'){
													?>
													<td><button id=<?php  echo $row["category_id"] ?> href="#" data-role="off" data-is=<?php  echo $row["category_id"] ?> >Active</button></td>
													</tr>

													<?php 
														}else{
															?>
															<td><button id=<?php  echo $row["category_id"] ?> href="#" data-role="on" data-is=<?php  echo $row["category_id"] ?> >In-active</button></td>
															</tr>
													<?php
														}
													?>
	...
	<script>
		$(document).ready(function(){
			...
			$(document).on('click','button[data-role=off]',function(){
				// 	displaycount();
				var category_id = this.id;
				var code = 0;
				//console.log("Disable",category_id);

				$.post('./php_form/category_status.php',{"id": category_id, "code": code});
				location.reload();

			});

			$(document).on('click','button[data-role=on]',function(){
				// 	displaycount();
				var category_id = this.id;
				var code = 1;
			...
			$('button[data-role=off]').css({"background-color" : "#0be881", "border" : "0", "color" : "#fff", "width" : "80px"});
			$('button[data-role=on]').css({"background-color" : "#ff3f34", "border" : "0", "color" : "#fff", "width" : "80px"})
