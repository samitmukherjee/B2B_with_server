...
<body class="menu-collapsed">
										...
										<table id="basic_datatable" class="display table table-bordered basic-data-table">
											<thead>
												<tr>
													...
													<th class="view-message">New Arrival status</th>
												</tr>
											</thead>
											<tbody>
											...
											<?php
												...
												while($row = mysqli_fetch_array($result)){
													...
													<?php 
													if($row["new_arrival_status"] == '1'){
													?>
													<td><button id=<?php  echo $row["product_id"] ?> href="#" data-role="new_arrival_off" data-is=<?php  echo $row["product_id"] ?> >Active</button></td>
													</tr>

													<?php 
														}else{
															?>
															<td><button id=<?php  echo $row["product_id"] ?> href="#" data-role="new_arrival_on" data-is=<?php  echo $row["product_id"] ?> >In-active</button></td>
															</tr>
													<?php
														}
													?>
	...
	<script>
		$(document).ready(function(){
			...
			$(document).on('click','button[data-role=new_arrival_off]',function(){
				// 	displaycount();
				var product_id = this.id;
				var code = 0;
				//console.log("Disable",category_id);

				$.post('./php_form/new_arrival_status.php',{"id": product_id, "code": code});
				location.reload();

			});

			$(document).on('click','button[data-role=new_arrival_on]',function(){
				// 	displaycount();
				var product_id = this.id;
				var code = 1;
			...
			$('button[data-role=new_arrival_off]').css({"background-color" : "#0be881", "border" : "0", "color" : "#fff", "width" : "80px"});
			$('button[data-role=new_arrival_on]').css({"background-color" : "#ff3f34", "border" : "0", "color" : "#fff", "width" : "80px"});
